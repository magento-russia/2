<?php
class Df_1C_Cml2_Export_Processor_Catalog_Attribute_Real
			...
			$this->{__METHOD__} =
				...
				&&
					!in_array($this->getAttribute()->getName(), array(
						 ...
						 * на витрине магазина.
						 * @see Df_1C_Cml2_Export_Processor_Catalog_Attribute_Url
						 */
						,'url_key'
						// «Видимость».
						// Думаю, клиентам передача значений этого свойства в 1С
	...
	private function getВариантыЗначений_SourceTable() {
		if (!isset($this->{__METHOD__})) {
			...
			foreach ($this->getAttribute()->getOptions($this->store()) as $option) {
				...
				/**
				 * По аналогии с
				 * @see Df_1C_Cml2_Import_Processor_ReferenceList::assignExternalIdToOptions()
				 */
				if (!$option->get1CId()) {
	...
	protected function _construct() {
		...
		$this
			...
			->_prop(self::$P__ATTRIBUTE, Df_Catalog_Model_Resource_Eav_Attribute::_C)
		;
	}
	/** @var string */
	private static $P__ATTRIBUTE = 'attribute';
	/**
	 * @used-by Df_1C_Cml2_Export_Document_Catalog::getProcessorForAttribute()
	 * @static
	...
	public static function i(
		...
		,Df_1C_Cml2_Export_Document_Catalog $document
	) {
