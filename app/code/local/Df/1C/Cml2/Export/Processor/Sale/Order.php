<?php
class Df_1C_Cml2_Export_Processor_Sale_Order extends Df_1C_Cml2_Export_Processor_Sale {
	/** @return void */
	...
	private function getCustomer() {
		if (!isset($this->{__METHOD__})) {
			$this->{__METHOD__} = Df_1C_Cml2_Export_Data_Entity_Customer::i($this->getOrder());
		}
	...
	private function getDocumentData_OrderItems() {
		...
		foreach ($this->getOrder()->getItemsCollection() as $item) {
			...
			if (Mage_Catalog_Model_Product_Type::TYPE_SIMPLE === $item->getProductType()) {
				$result[]= Df_1C_Cml2_Export_Processor_Sale_Order_Item::i($item)->getDocumentData();
			}
		}
