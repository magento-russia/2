...
class Df_1C_Cml2_Action_Catalog_Import extends Df_1C_Cml2_Action_Catalog {
	...
	/** @return Df_1C_Cml2_Action_Catalog_Import */
	private function importCategories() {
		...
		if (!$count) {
			...
			foreach ($this->getCollections()->getCategories() as $category) {
				/** @var Df_1C_Cml2_Import_Data_Entity_Category $category */
				Df_1C_Cml2_Import_Processor_Category::i(
	...
	private function importProductsConfigurable() {
		...
		if (!$countParent) {
			...
			foreach ($this->getCollections()->getOffersConfigurableParent() as $offer) {
				...
				Df_1C_Cml2_Import_Processor_Product_Type_Configurable::p($offer);
			}
	...
	private function importProductsConfigurablePartImages() {
		foreach ($this->getCollections()->getOffers() as $offer) {
			...
			if ($offer->isTypeConfigurableParent()) {
				Df_1C_Cml2_Import_Processor_Product_Part_Images::i($offer)->process();
			}
		}
		return $this;
	}
	...
	private function importProductsSimple() {
		...
		if (!$count) {
			...
			foreach ($this->getCollections()->getOffersSimple() as $offer) {
				/** @var Df_1C_Cml2_Import_Data_Entity_Offer $offer */
				Df_1C_Cml2_Import_Processor_Product_Type_Simple::i($offer)->process();
			}
	...
	private function importReferenceLists() {
		...
		foreach ($this->getCollections()->getAttributes() as $attribute) {
			/** @var Df_1C_Cml2_Import_Data_Entity_Attribute $attribute */
			// Обратываем только свойства, у которых тип значений — «Справочник».
			if ($attribute instanceof Df_1C_Cml2_Import_Data_Entity_Attribute_ReferenceList) {
				/** @var Df_1C_Cml2_Import_Data_Entity_Attribute_ReferenceList $attribute */
				Df_1C_Cml2_Import_Processor_ReferenceList::i($attribute)->process();
			}
		}
