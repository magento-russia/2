<?php
class Df_1C_Cml2_File extends Df_Core_Model {
	/** @return string */
	...
	public function getXmlDocument() {
		return Df_1C_Cml2_FileSystem::s()->getXmlDocumentByRelativePath($this->getPathRelative());
	}

	/** @return Df_1C_Cml2_Import_Data_Document_Catalog */
	public function getXmlDocumentAsCatalog() {
		if (!isset($this->{__METHOD__})) {
			df_assert($this->getXmlDocument() instanceof Df_1C_Cml2_Import_Data_Document_Catalog);
	...
	public function getXmlDocumentAsOffers() {
		if (!isset($this->{__METHOD__})) {
			df_assert($this->getXmlDocument() instanceof Df_1C_Cml2_Import_Data_Document_Offers);
			$this->{__METHOD__} = $this->getXmlDocument();
		}
		return $this->{__METHOD__};
	}
