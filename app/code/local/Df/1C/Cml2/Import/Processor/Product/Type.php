<?php
abstract class Df_1C_Cml2_Import_Processor_Product_Type
	extends Df_1C_Cml2_Import_Processor_Product {
	/**
	...
	protected function getImporter() {
		if (!isset($this->{__METHOD__})) {
			if (!$this->getExistingMagentoProduct()) {
				/** @var Df_1C_Cml2_Import_Data_Document_Offers $document */
	...
	protected function getProductDataNewOrUpdateBase() {
		...
		$result = array(
			Df_1C_Const::ENTITY_EXTERNAL_ID => $this->getEntityOffer()->getExternalId()
			,'sku' => $this->getSku()
		 ...
		 * @uses Df_1C_Cml2_Import_Data_Entity::getName()
		 * может вернуть null.
	...
	private function getDescription() {
		return $this->getDescriptionAbstract(
			...
			,$fieldsToUpdate = array(
				Df_1C_Config_Source_WhichDescriptionFieldToUpdate::V__DESCRIPTION
				,Df_1C_Config_Source_WhichDescriptionFieldToUpdate::V__BOTH
			)
		);
	}
	...
	private function getDescriptionShort() {
		return $this->getDescriptionAbstract(
			...
			,$fieldsToUpdate = array(
				Df_1C_Config_Source_WhichDescriptionFieldToUpdate::V__SHORT_DESCRIPTION
				,Df_1C_Config_Source_WhichDescriptionFieldToUpdate::V__BOTH
			)
		);
	}
		...
		if ($this->getDocumentCurrentAsOffers()->isBase()) {
			$result = array_merge($result
				...
				,$this->getProductDataNewOrUpdateAttributeValues(array(
					Df_1C_Cml2_Import_Data_Entity_AttributeValue_ProcurementDate::i(
						$this->getEntityOffer()
					)
					,Df_1C_Cml2_Import_Data_Entity_AttributeValue_Barcode::i(
						$this->getEntityOffer()
					)
				))
	...
	/**
	 * @param Df_1C_Cml2_Import_Data_Entity_AttributeValue[] $attributeValues
	 * @return array(string => string|int|float|bool|null)
	 */
	private function getProductDataNewOrUpdateAttributeValues($attributeValues) {
		...
		foreach ($attributeValues as $value) {
			/** @var Df_1C_Cml2_Import_Data_Entity_AttributeValue $value */
	...
	private function getTierPricesInImporterFormat() {
		if (!isset($this->{__METHOD__})) {
			/** @var mixed[] $result  */
			$result = array();
			/** @var Df_1C_Cml2_Import_Data_Entity_OfferPart_Price|null $mainPrice */
			$mainPrice = $this->getEntityOffer()->getPrices()->getMain();
			foreach ($this->getEntityOffer()->getPrices()->getItems() as $price) {
				/** @var Df_1C_Cml2_Import_Data_Entity_OfferPart_Price $price */
