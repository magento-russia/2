<?php
class Df_1C_Cml2_Import_Data_Entity_Order_Item extends Df_1C_Cml2_Import_Data_Entity {
	...
	public function getProduct() {
		if (!isset($this->{__METHOD__})) {
			...
			$result = $product->loadByAttribute(Df_1C_Const::ENTITY_EXTERNAL_ID, $this->getExternalId());
			if (false === $result) {
				/**
				...
				if ($result && $result->getId()) {
					...
					// Добавляем.
					Df_1C_Cml2_Processor_Product_AddExternalId::p($result, $this->getExternalId());
				}
			}
			if (!$result || !$result->getId()) {
				df_error(
