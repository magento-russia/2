...
class Df_Shipping_Model_Response extends Df_Core_Model {
	...
	public function json($path = null, $defaultValue = null) {
		...
		if (is_null($path)) {
			...
			if (!isset($this->_jsonCache[$path])) {
				$this->_jsonCache[$path] = df_a_deep($this->_jsonDecoded, $path, $defaultValue);
			}
	 ...
	 * @return void
	 */
	public function log() {rm_report($this->getLogFileNameTemplate(), $this->report());}
	...
	public function report() {
		return
			...
			? rm_print_params($this->_jsonDecoded)
			: $this->text()
		;
	}
	 ...
	 * @return Df_Core_Sxe|Df_Core_Sxe[]|null
	 */
	public function xml($path = null, $all = false) {
		...
		/** @var Df_Core_Sxe|Df_Core_Sxe[] $result */
		$result = $this->_xml;
		if (!is_null($path)) {
	...
	private function getLogFileNameTemplate() {
		return strtr('{module}-{ordering}.{extension}', array(
			'{extension}' => $this->_type, '{module}' => rm_module_id($this->getRequest(), '.')
		));
	}
	...
	protected function _construct() {
		...
		$this
			...
			->_prop(self::P__TEXT, RM_V_STRING_NE)
		;
	}
