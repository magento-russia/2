...
class Df_Shipping_Rate_Request extends Mage_Shipping_Model_Rate_Request {
	...
	public function checkCountryOriginIs($countryIso2Code) {
		...
		if ($countryIso2Code !== $this->getOriginCountryId()) {
			$this->throwException(
				...
				, rm_country($countryIso2Code)->getNameInFormOrigin()
			);
		}
		return $this;
	}
	...
	public function getDeclaredValueInHryvnias() {
		if (!isset($this->{__METHOD__})) {
			$this->{__METHOD__} = rm_currency_h()->convertFromBaseToHryvnias($this->declaredValueBase());
		}
		return $this->{__METHOD__};
	}
	...
	public function getDeclaredValueInRoubles() {
		if (!isset($this->{__METHOD__})) {
			$this->{__METHOD__} = rm_currency_h()->convertFromBaseToRoubles($this->declaredValueBase());
		}
		return $this->{__METHOD__};
	}
	...
	public function getDeclaredValueInTenge() {
		if (!isset($this->{__METHOD__})) {
			$this->{__METHOD__} = rm_currency_h()->convertFromBaseToTenge($this->declaredValueBase());
		}
		return $this->{__METHOD__};
	}
	...
	public function getDimensionMaxRoughInMetres() {
		if (!isset($this->{__METHOD__})) {
			$this->{__METHOD__} = rm_length()->inMetres(max($this->getDimensionsRough()));
		}
		return $this->{__METHOD__};
	}
	...
	public function getDimensionMinRoughInMetres() {
		if (!isset($this->{__METHOD__})) {
			$this->{__METHOD__} = rm_length()->inMetres(min($this->getDimensionsRough()));
		}
		return $this->{__METHOD__};
	}
	...
	public function getDimensionsRoughInMetres() {
		if (!isset($this->{__METHOD__})) {
			$this->{__METHOD__} = rm_length()->inMetres($this->getDimensionsRough());
		}
		return $this->{__METHOD__};
	}
	...
	public function getDimensionsSumRoughInMetres() {
		if (!isset($this->{__METHOD__})) {
			$this->{__METHOD__} = rm_length()->inMetres(array_sum($this->getDimensionsRough()));
		}
		return $this->{__METHOD__};
	}
	...
	public function getHeightRoughInCentimeters() {
		if (!isset($this->{__METHOD__})) {
			$this->{__METHOD__} = rm_length()->inCentimetres($this->getHeightRough());
		}
		return $this->{__METHOD__};
	}
	...
	public function getLengthRoughInCentimeters() {
		if (!isset($this->{__METHOD__})) {
			$this->{__METHOD__} = rm_length()->inCentimetres($this->getLengthRough());
		}
		return $this->{__METHOD__};
	}
	...
	public function getQty() {
		if (!isset($this->{__METHOD__})) {
			...
			$this->{__METHOD__} = array_sum(rm_int_simple(df_each($this->getQuoteItemsSimple(), 'getQty')));
		}
		return $this->{__METHOD__};
	}
	...
	public function getVolumeInCubicMetres() {
		if (!isset($this->{__METHOD__})) {
			...
			foreach ($this->getQuoteItemsSimple() as $quoteItem) {
				...
				$productVolume = array_product(rm_length()->inMetres(
	...
	public function getWeightInGrammes() {
		if (!isset($this->{__METHOD__})) {
			$this->{__METHOD__} = rm_weight()->inGrammes($this->getPackageWeight());
		}
		return $this->{__METHOD__};
	}
	...
	public function getWidthRoughInCentimeters() {
		if (!isset($this->{__METHOD__})) {
			$this->{__METHOD__} = rm_length()->inCentimetres($this->getWidthRough());
		}
		return $this->{__METHOD__};
	}
	...
	public function throwException($message) {
		...
		df_error(new Df_Shipping_Exception_MethodNotApplicable(
			df_no_escape($this->evaluateMessage(rm_format($arguments))
		)));
	}
	 ...
	 * @return bool
	 */
	private function isMoscow(array $variants) {return in_array('МОСКВА', df_t()->strtoupper($variants));}
