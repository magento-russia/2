...
class Df_YandexMarket_Category_Excel_Processor_Row extends Df_Core_Model {
	...
	public function process() {
		...
		foreach ($this->getRow() as $nodeName) {
			...
			/** @var string $nodeName */
			/** @var Df_YandexMarket_Category_Node $node */
			$node = null;
			...
			if (is_null($node)) {
				$node =
					new Df_YandexMarket_Category_Node(
						$data = array(
							'id' => df_uid()
							,'name' => $nodeName
						)
	...
	protected function _construct() {
		parent::_construct();
		$this
			->_prop(self::$P__TREE, Df_YandexMarket_Category_Tree::class)
	...
	/**
	 * @static
	 * @param Df_YandexMarket_Category_Tree $tree
	 * @param string[] $row
	 * @return Df_YandexMarket_Category_Excel_Processor_Row
	 */
	public static function i(Df_YandexMarket_Category_Tree $tree, array $row) {
