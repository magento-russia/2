...
class Df_Reward_Adminhtml_Customer_RewardController extends Mage_Adminhtml_Controller_Action {
	...
	public function historyAction() {
		$this->getResponse()->setBody(
			Df_Reward_Block_Adminhtml_Customer_Edit_Tab_Reward_History::i(
	...
	public function historyGridAction() {
		$this->getResponse()->setBody(
			Df_Reward_Block_Adminhtml_Customer_Edit_Tab_Reward_History_Grid::i(
				$this->getRequest()->getParam('id', 0)
			)->toHtml()
		);
	}
	...
	public function deleteOrphanPointsAction()
	{
		...
		if ($customerId) {
			try {
				...
				rm_session()->addSuccess(df_h()->reward()->__('Orphan points removed successfully.'));
			} catch (Exception $e) {
	...
	{
		return df_mage()->admin()->session()->isAllowed('df_reward/balance');
	}
}