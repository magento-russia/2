<?php
class Df_Cms_Block_Adminhtml_Cms_Page_Version_Edit extends Mage_Adminhtml_Block_Widget_Form_Container {
	/**
	 ...
	 * @override
	 * @return Df_Cms_Block_Adminhtml_Cms_Page_Version_Edit
	 */
				...
				->_addButton(
					...
					,array(
						...
						,'onclick' =>
							rm_sprintf(
								'setLocation(%s);'
								,df_quote_single($this->getNewRevisionUrl())
							)
						,'class' => 'new'
					)
				)
	...
	public function getHeaderText() {
		...
		$versionLabel = df_text()->escapeHtml(Mage::registry('cms_page_version')->getLabel());
		if (!$versionLabel) {
			$versionLabel = df_h()->cms()->__('N/A');
		}
		return
			df_h()->cms()->__(
				"Edit Page '%s' Version '%s'"
				,df_text()->escapeHtml(Mage::registry('cms_page')->getTitle())
				,$versionLabel
			)
		;
	}
