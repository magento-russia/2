...
class PelTiff {
  ...
  function __construct($data = false) {
	...
	if (is_string($data)) {
	 ...
	 $this->loadFile($data);
	} else if ($data instanceof PelDataWindow) {
  ...
  function getBytes($order = PelConvert::LITTLE_ENDIAN) {
	...
	if ($this->ifd != null) {
	   ...
	   * use this to calculate offsets from the entries to their data,   * all those offsets are absolute offsets counted from the
