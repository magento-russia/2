...
class PelJpeg {
  ...
  function __construct($data = false) {
	...
	if (is_string($data)) {
	 ...
	 $this->load($data);
	} else if (is_resource($data) && get_resource_type($data) == 'gd') {
  ...
  function load(PelDataWindow $d) {
	...
	while ($d->getSize() > 0) {
	  ...
	  if ($marker == PelJpegMarker::SOI || $marker == PelJpegMarker::EOI) {
		...
		} else if ($marker == PelJpegMarker::COM) {
...
//				&&
			...
			if ($length != $d->getSize()) {
			  Pel::maybeThrow(new PelException('Found trailing content ' .
											   'after EOI: %d bytes',   $d->getSize() - $length));
  ...
  function setExif (PelExif $exif) {
	...
	for($i = 0; $i < count($this->sections); $i++) {
	  if ($this->sections[$i][0] == PelJpegMarker::APP0) {
		$app0_offset = $i;
	  } else if ($this->sections[$i][0] == PelJpegMarker::APP1) {
		$app1_offset = $i;
		break;
	  }
	}
  ...
  function __toString() {
	...
	for($i = 0; $i < count($this->sections); $i++) {
	  ...
	  if ($c instanceof PelExif) {
		...
		$str .= $c->__toString() . "\n";
	  } else if ($c instanceof PelJpegComment) {
