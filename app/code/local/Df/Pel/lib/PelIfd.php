	...
	for($i = 0; $i < $n; $i++) {
	  ...
	  case PelTag::INTEROPERABILITY_IFD_POINTER:
		...
		if ($tag == PelTag::EXIF_IFD_POINTER)
		  $type = PelIfd::EXIF;
		else if ($tag == PelTag::GPS_INFO_IFD_POINTER)
		  $type = PelIfd::GPS;
		else if ($tag == PelTag::INTEROPERABILITY_IFD_POINTER)
		  $type = PelIfd::INTEROPERABILITY;
	...
	if ($offset > 0 && $length > 0) {
	  ...
	  if ($offset + $length > $d->getSize()) {
		Pel::maybeThrow(new PelIfdException('Thumbnail length %d bytes ' .
											'adjusted to %d bytes.',$length,$d->getSize() - $offset));
...
PelTag::CFA_REPEAT_PATTERN_DIM, 
  ...
  function getBytes($offset, $order) {
	 ...
	 * preceding this IFD, the bytes needed for the count of entries, * the entries themselves (and sub entries), the extra data in the
	...
	foreach ($this->sub as $type => $sub) {
	  ...
	  else if ($type == PelIfd::GPS)
		$tag = PelTag::GPS_INFO_IFD_POINTER;
	  else if ($type == PelIfd::INTEROPERABILITY)
